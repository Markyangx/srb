...
public class LendController {
    ...
    public ResultWarrp list() {
        ...
        return ResultWarrp.ok().data("lendList", lendList);
    }
    ...
    public ResultWarrp show(
        ...
        return ResultWarrp.ok().data("lendDetail", lendDetail);
    }

    @ApiOperation("查询账户余额")
    @GetMapping("/auth/getAccount")
    public ResultWarrp getAccount(HttpServletRequest request){
        String token = request.getHeader("token");
        Long userId = JwtUtils.getUserId(token);
        BigDecimal account = userAccountService.getAccount(userId);
        return ResultWarrp.ok().data("account", account);
    }

    @ApiOperation("计算投资收益")
    @GetMapping("/getInterestCount/{invest}/{yearRate}/{totalmonth}/{returnMethod}")
    public ResultWarrp getInterestCount(
        ...
        return ResultWarrp.ok().data("interestCount", interestCount);
    }
}
